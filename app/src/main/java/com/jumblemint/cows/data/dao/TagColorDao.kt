...
interface TagColorDao {
    @Query("SELECT * FROM tag_colors WHERE isActive = 1 ORDER BY name ASC")
    fun getAllActiveTagColors(): Flow<List<TagColor>>

    @Query("SELECT * FROM tag_colors ORDER BY isDefault DESC, name ASC")
    suspend fun getAllSync(): List<TagColor>

    @Query("SELECT * FROM tag_colors ORDER BY name ASC")
    ...
    @Query("SELECT * FROM tag_colors WHERE name = :name AND isActive = 1")
